...
public class ShoppingCart {
     ...
     * @throws IllegalAccessException หากไม่พบสินค้านั้นในแคตตาล็อก
     */
    public void addItem(String productId, int quantity) throws ProductNotFoundException, IllegalAccessException{
        // findByid จะโยน Exception เองถ้าไม่เจอสินค้า
        Product p = productCatalog.findById(productId);
        if (quantity <= 0) {
            //ใช้ Unchecked Exception สำหรับข้อผิดพลาดของโปรแกรมเมอร์ (Precondtion violation)
            throw new IllegalAccessException("Quantity must be positive");
        }
        for (CartItem item : items) {
